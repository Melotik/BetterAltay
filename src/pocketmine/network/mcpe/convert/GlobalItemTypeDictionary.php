...
final class GlobalItemTypeDictionary{
	...
	private static function make() : self{
		...
		foreach (ProtocolInfo::ACCEPTED_PROTOCOL_WITH_VALUE as $i => $v){
			$data = file_get_contents(\pocketmine\RESOURCE_PATH . "/vanilla/required_item_list_$v.json");
			...
			$dictionaries[$v] = new ItemTypeDictionary($params);
		}
		return new self($dictionaries);
	}

	/**
	 * @param ItemTypeEntry[] $dictionaries
	 */
	...
	public static function sortByProtocol(array $players) : array{
		...
		foreach($players as $player){
			$dictionaryProtocol = self::getDictionaryProtocol($player->getProtocol());
	 ...
	 * @phpstan-return list<ItemTypeDictionary>
	 */
